...
Game.Level2.prototype = {
  ...
  create: function(game) {
    ...
    //Link tileset to map2
    map.addTilesetImage('tileset');

    //Initialise the layers and assign to variables
    layer = map.createLayer('Background');
    platformslayer = map.createLayer('Platforms');

    layer.resizeWorld();

    //Make platforms layer collideable
    ...
    //Dont allow outside the canvas bounds
    player.body.collideWorldBounds = true;

    //Button to start the next level
    ...
    //Asign keyboard controls to controls variable
    controls = this.input.keyboard.createCursorKeys();

    //Add three mummies using the Mummy function
    ...
    //Add three items using the Item function
    ...
    //Show mobile controls if the device accessing the game is not a desktop device
  ...
  update: function(game) {
    ...
    //Make all objects collide with platforms layer
    ...
    //Check if the main character collides with any of the mummies and reset the level
    if (checkOverlap(player, mummy1.mummy) || checkOverlap(player, mummy2.mummy) || checkOverlap(player, mummy3.mummy)) {
      this.state.start('Level2');
    }

    //Check if the item has overlapped and show info
    ...
    //If all items collected, show next level button
