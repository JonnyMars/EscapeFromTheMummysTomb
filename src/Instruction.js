...
Game.Instruction.prototype = {
  ...
  create: function(game) {
    ...
    var title = game.add.text(315, 44, "INSTRUCTIONS", {fill: '#fff', fontSize:'25px', align:'center'});
    title.anchor.setTo(0.5, 0.5);
    var info = game.add.text(180  , 85, "Controls:", {fill: '#fff', fontSize:'23px', align:'center'});
    ...
    if (!Phaser.Device.desktop){
      ...
      left.angle = 270;
      this.add.text(140, 200, "Move Left", {fill: '#ffff00', fontSize:'20px', align:'center'});

      var right = this.add.sprite(320, 160,'arrowwhite');
      right.anchor.setTo(0.5, 0.5);
      right.angle = 90;
      this.add.text(265, 200, "Move Right", {fill: '#ffff00', fontSize:'20px', align:'center'});

      var jump = this.add.sprite(455, 160,'arrowwhite');
      jump.anchor.setTo(0.5, 0.5);
      this.add.text(428, 200, "Jump", {fill: '#ffff00', fontSize:'20px', align:'center'});
    } else {
      var arrowkeys = this.add.sprite(310, 165, 'arrowkeys');
      arrowkeys.anchor.setTo(0.5, 0.5);
      arrowkeys.scale.setTo(0.9, 0.9);
      this.add.text(130, 180, "Move Left", {fill: '#ffff00', fontSize:'20px', align:'center'});
      this.add.text(400, 180, "Move Right", {fill: '#ffff00', fontSize:'20px', align:'center'});
      this.add.text(285, 80, "Jump", {fill: '#ffff00', fontSize:'20px', align:'center'});
    }
    ...
    this.add.text(130, 245, "Collect:", {fill:'#fff', fontSize: '23px' })
    var item = this.add.sprite(140, 290, 'item', 0);
    this.add.text(220, 315, "x 3", {fill:'#fff', fontSize: '25px' })
    item.scale.setTo(2, 2)
    this.add.text(335, 245, "Avoid:", {fill:'#fff', fontSize: '23px' })
